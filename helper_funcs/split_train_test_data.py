...
def main():
    ...
    for f in train_files:
        os.system(f"cp {os.path.join(source_dir_audio, f)} {os.path.join(train_dir, f)}")
        txt_file = f.split(".")[0] + ".txt"
        os.system(f"cp {os.path.join(source_dir_anotations, txt_file)} {os.path.join(train_dir, txt_file)}")

    for f in test_files:
        os.system(f"cp {os.path.join(source_dir_audio, f)} {os.path.join(test_dir, f)}")
        txt_file = f.split(".")[0] + ".txt"
        os.system(f"cp {os.path.join(source_dir_anotations, txt_file)} {os.path.join(test_dir, txt_file)}")
